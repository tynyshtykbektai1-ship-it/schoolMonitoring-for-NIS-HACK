...
import json
try:
    from backend.hackathon_features import create_hackathon_router
except ImportError:
    from hackathon_features import create_hackathon_router
...
@app.get("/")
async def root():
    frontend_index_v2 = os.path.join("frontend", "index_v2.html")
    if os.path.exists(frontend_index_v2):
        return FileResponse(frontend_index_v2)
    return JSONResponse({"status": "error", "message": "frontend/index_v2.html not found."})
...
async def root_legacy():
    ...
    return JSONResponse({"status": "ok", "message": "Frontend not found. Open /docs for API."})
...
app.include_router(
    create_hackathon_router(
        get_violations=lambda: active_violations,
        get_active_students=lambda: active_students,
        get_last_frames=lambda: last_frames,
    )
)
